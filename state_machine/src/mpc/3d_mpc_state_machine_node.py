#!/usr/bin/env python3
# ### HJ : Phase X — file lives in state_machine/src/mpc/; siblings
# (state_transitions_mpc, states_mpc) are here too, but shared `states_types`
# is one level up in state_machine/src/. Prepend parent dir to sys.path so
# `from states_types import ...` and `import states_mpc` both resolve.
import os
import sys
sys.path.insert(
...
class StateMachine:
    ...
    def __init__(self, name) -> None:
        ...
        # ### HJ : 2026-04-24 — asymmetric RECOVERY→GB exit hysteresis.
        # Tight thresholds require MPC's NO_OBS path to have actually
        # converged ego onto the raceline for K ticks before handing over.
        # Overridable via rosparam for tuning without restart.
        ### HJ : 2026-04-26 — 0.05 → 0.10. "GB 붙는 정도 10cm" — recovery
        ###      releases to GB tracking once |ego_n| < 0.10 m for K ticks.
        self._rc_exit_tight_m = rospy.get_param(
            "state_machine/rc_exit_tight_m", 0.10)
        self._rc_exit_tight_deg = rospy.get_param(
            "state_machine/rc_exit_tight_deg", 10)
        # ### HJ : 2026-04-24 — default 8→2. With solver's q_n k-ramp in
        # recovery profile the MPC tail now actually lands on GB (n→0 at
        # k=N forced by q_n + q_n_ramp = 48 stage + q_n_term=80 terminal),
        # so we trust the handoff after just a couple of ticks of tight-
        # close proximity. Set to 1 for instantaneous handoff if trajectory
        # quality holds up.
        self._rc_exit_hyst_ticks = rospy.get_param(
        ...
        # frenet |d_path - d_obs| (which is raceline-normal — misleading on
        # curves). Legacy non-MPC paths keep the d-based check.
        # xy_clearance_m: obstacle-to-path minimum clearance below which
        # the path is deemed blocked (triggers TRAILING fallback).
        self._use_xy_clearance_check = bool(rospy.get_param(
            "state_machine/use_xy_clearance_check", True))
        self._xy_clearance_m = float(rospy.get_param(
            "state_machine/xy_clearance_m", 0.25))
        ...
        # ### HJ : Phase X (refactored) — unified MPC topic. MPC publishes a
        # single trajectory tagged via ot_line ("avoid" | "recover"). We fan
        # it into BOTH cur_avoidance_wpnts and cur_recovery_wpnts so the
        # existing OVERTAKE/RECOVERY state code paths consume the same data
        # without structural change (Phase C will unify them).
        rospy.Subscriber("/planner/mpc/wpnts", OTWpntArray, self.mpc_wpnts_cb)
        ...
        # ### HJ : Phase X — live debug channel for rostopic echo monitoring.
        # Keys: sm_state, path_source, n_ego, mpc_mode_echo, trail_targets_n,
        # ot_age_ms, rc_age_ms.
        self.mpc_debug_pub = rospy.Publisher(
            "/state_machine_mpc/debug", String, queue_size=1)
        self._mpc_path_source = "GB"   # GB | MPC_OT | MPC_RC
        self._mpc_last_mode_echo = "-"
    ...
    def recovery_wpnts_cb(self, data: WpntArray):
        ...
        # ### HJ : Phase X — stamp for freshness check in the debug pub.
        self._mpc_rc_last_stamp = data.header.stamp if data.header else None

    def avoidance_cb(self, data: OTWpntArray):
        """splini waypoints (legacy)"""
        if len(data.wpnts) !=0:
            self.update_velocity(data, self.cur_avoidance_wpnts.vel_planner_safety_factor)
        self.avoidance_wpnts = data
        # ### HJ : Phase X — stamp + ot_line tag for debug monitoring.
        self._mpc_ot_last_stamp = data.header.stamp if data.header else None
        self._mpc_last_mode_echo = getattr(data, 'ot_line', '-') or '-'

    def mpc_wpnts_cb(self, data: OTWpntArray):
        """### HJ : Phase X (refactored) — unified MPC trajectory sink.

        A single MPC publishes to /planner/mpc/wpnts with ot_line ∈
        {"avoid", "recover"} tag. We fan-out into BOTH:
          - self.avoidance_wpnts (drives OVERTAKE state path)
          - self.recovery_wpnts  (drives RECOVERY state path, as WpntArray)
        so the legacy state-transition code (which picks between them based
        on ego_n / close_to_raceline) keeps working without structural
        change. Phase C will flatten this into a single cur_mpc_wpnts.

        Also marks cur_{avoidance,recovery}_wpnts.from_mpc=True so the
        wpnt getter helpers skip the GB-padding step (MPC raw passthrough).
        """
        if len(data.wpnts) == 0:
            return
        # Update velocity for OT path (uses avoidance safety factor).
        self.update_velocity(data, self.cur_avoidance_wpnts.vel_planner_safety_factor)
        # Fan to avoidance cache.
        self.avoidance_wpnts = data
        # Build a sister WpntArray view for the recovery cache.
        rc = WpntArray()
        rc.header = data.header
        rc.wpnts = list(data.wpnts)
        self.recovery_wpnts = rc
        # Provenance tags — used by get_splini_wpts / get_recovery_wpts to
        # skip GB padding. We set here (before initialize_traj) so subsequent
        # _check_latest_wpnts → initialize_traj keeps the flag alive.
        try:
            self.cur_avoidance_wpnts.from_mpc = True
            self.cur_recovery_wpnts.from_mpc = True
        except Exception:
            pass
        # Debug bookkeeping — both channels age in sync now.
        stamp = data.header.stamp if data.header else None
        self._mpc_ot_last_stamp = stamp
        self._mpc_rc_last_stamp = stamp
        self._mpc_last_mode_echo = getattr(data, 'ot_line', '-') or '-'
    ...
    def _check_overtaking_mode(self) -> bool:
        ...
        # ### HJ : 2026-04-25 — xy clearance for MPC paths (Euclidean),
        # frenet d for legacy sources. See _check_free_xy docstring.
        from_mpc = getattr(self.cur_avoidance_wpnts, 'from_mpc', False)
        if from_mpc and getattr(self, '_use_xy_clearance_check', True):
            free_check = self._check_free_xy(self.cur_avoidance_wpnts)
        else:
    ...
    # ### HJ : 2026-04-25 — xy-distance path-clear check. Replaces the
    # frenet |d_path - d_obs| comparison (which is raceline-normal, wrong
    # on curves) with true Euclidean min-distance from obstacle xy to any
    # waypoint xy on the path. Only scans wpnts within an s-window around
    # the obstacle for speed. Returns True if every obstacle's closest
    # path point is at least `xy_clearance_m` away from obstacle edge.
    def _check_free_xy(self, wpnts_data, xy_clearance_m=None) -> bool:
        if wpnts_data is None or not wpnts_data.is_init:
            return False
        if xy_clearance_m is None:
        ...
        for obs in obstacles:
            ...
            gap = (obs_s - ego_s) % tl
            if gap > max_horizon:
                continue   # obstacle beyond path horizon — skip
            obs_xy = np.array([obs.x_m, obs.y_m], dtype=np.float64)
            dxy = arr[:, :2] - obs_xy[None, :]
    ...
    def get_splini_wpts(self) -> WpntArray:
        ...
        # ### HJ : 2026-04-24 — MPC raw passthrough. When the source came
        # from /planner/mpc/wpnts we do NOT pad with GB waypoints: the SM
        # state machine is meant to switch between GB or MPC cleanly, not
        # stitch the two at the MPC horizon tail (that tail-stitch is the
        # user-reported "울퉁불퉁" seam). If MPC output is shorter than
        # n_loc_wpnts the controller sees a short path — log that so we
        # notice lookahead shortage at high speed.
        if getattr(wpnts, 'from_mpc', False):
            if len(avoidance_wpnts) < max(10, int(self.n_loc_wpnts * 0.3)):
                rospy.logwarn_throttle(
                    1.0,
                    "[%s] MPC avoidance path short: %d/%d wpnts (no GB padding; raise mpc horizon if controller starves)",
                    self.name, len(avoidance_wpnts), self.n_loc_wpnts)
            return avoidance_wpnts

        if len(avoidance_wpnts) < self.n_loc_wpnts:
            # Non-MPC source (legacy static_avoidance) — keep GB padding.
    ...
    def get_recovery_wpts(self) -> WpntArray:
        """Obtain the recovery path. Under Phase X (refactored) this comes
        from the unified MPC topic (/planner/mpc/wpnts) and we return the
        MPC output raw — no GB padding. Legacy sources still get padded.
        """
        if self.cur_recovery_wpnts.is_init:
            ...
            # ### HJ : 2026-04-24 — MPC raw passthrough (see get_splini_wpts).
            if getattr(self.cur_recovery_wpnts, 'from_mpc', False):
                if len(wpnts) < max(10, int(self.n_loc_wpnts * 0.3)):
                    rospy.logwarn_throttle(
                        1.0,
                        "[%s] MPC recovery path short: %d/%d wpnts (no GB padding)",
                        self.name, len(wpnts), self.n_loc_wpnts)
                return wpnts
    ...
    def _rewrite_trailing_to_local_arc(self):
        """### HJ : 2026-04-25 — Replace trailing_target s_center with
        ego_global_s + (arc length along local_wpnts from ego to target
        projection). Controller's gap = (s_center - ego_s) % track_length
        then yields arc_length directly — no controller change needed.

        Behind-ego obstacles are filtered out (popped from list) so the
        controller never trails on a phantom.
        """
        if not getattr(self, '_trailing_gap_local_arc', True):
            return  # feature disabled via rosparam
        tgts = self.behavior_strategy.trailing_targets
        if not tgts:
            return
        lw = self.behavior_strategy.local_wpnts
        if lw is None or len(lw) < 2:
            return   # fall back to legacy global-s (no rewrite)
        if self.current_position is None:
            return
        lxy = np.array([(w.x_m, w.y_m) for w in lw], dtype=np.float64)
        ego_xy = np.array(self.current_position[:2], dtype=np.float64)
        ego_dists = np.linalg.norm(lxy - ego_xy, axis=1)
        ego_idx = int(np.argmin(ego_dists))
        # Precompute cumulative arc from wpnt[0]
        seg_len = np.linalg.norm(np.diff(lxy, axis=0), axis=1)
        cum = np.concatenate([[0.0], np.cumsum(seg_len)])  # shape (N,)
        kept = []
        for tgt in tgts:
            obs_xy = np.array([tgt.x_m, tgt.y_m], dtype=np.float64)
            obs_dists = np.linalg.norm(lxy - obs_xy, axis=1)
            obs_idx = int(np.argmin(obs_dists))
            # If target is behind ego on local path, drop it
            if obs_idx <= ego_idx:
                continue
            arc = float(cum[obs_idx] - cum[ego_idx])
            # Overwrite s_center so controller's formula gives `arc`.
            # (opp - ego) % track_length = ((ego+arc) - ego) % tl = arc
            # when arc < tl, which is always true for local horizon.
            tgt.s_center = float(self.cur_s + arc)
            kept.append(tgt)
    ...
    def loop(self):
        ...
        # ### HJ : Phase X — debug publish for rostopic echo monitoring.
        # path_source derived from local_wpnts_src (StateType) → GB/MPC_OT/MPC_RC.
        try:
            import json as _json
            src = self.local_wpnts_src
            if src == StateType.OVERTAKE:
                path_source = 'MPC_OT'
            elif src == StateType.RECOVERY:
                path_source = 'MPC_RC'
            else:
                path_source = 'GB'
            self._mpc_path_source = path_source
            ...
            def _age_ms(stamp):
                if stamp is None:
                    return None
                try:
                    return round((now - stamp).to_sec() * 1000.0, 1)
                except Exception:
                    return None
            ot_age = _age_ms(getattr(self, '_mpc_ot_last_stamp', None))
            rc_age = _age_ms(getattr(self, '_mpc_rc_last_stamp', None))

            trail_n = len(self.behavior_strategy.trailing_targets) \
                if self.behavior_strategy and self.behavior_strategy.trailing_targets else 0

            payload = {
                'sm_state': str(self.cur_state.name if hasattr(self.cur_state, 'name') else self.cur_state),
                'path_source': path_source,
                'mpc_mode_echo': str(self._mpc_last_mode_echo),
                'trail_targets_n': int(trail_n),
                'ot_age_ms': ot_age,
                'rc_age_ms': rc_age,
                'n_wpnts': int(len(local_wpnts) if local_wpnts else 0),
            }
            self.mpc_debug_pub.publish(String(data=_json.dumps(payload)))
        except Exception as _dbg_e:  # pragma: no cover
            rospy.logwarn_throttle(
                5.0, '[mpc_sm] debug publish failed: %s', _dbg_e)
...
if __name__ == "__main__":
    # ### HJ : Phase X — fork of 3d_state_machine_node. Node name differs so
    # the MPC-paired launch (`3d_mpc_headtohead.launch`) can coexist with the
    # original in testing (one can be killed while the other runs). Never
    # start both at once against the same controller.
    name = "mpc_state_machine"
